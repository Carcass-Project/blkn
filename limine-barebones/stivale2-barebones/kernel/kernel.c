...
static struct stivale2_header_tag_smp smp = { .tag = { .identifier = STIVALE2_HEADER_TAG_SMP_ID, .next = 0 }, .flags = 0};
...
void _start(struct stivale2_struct *stivale2_struct) {
    ...
    struct stivale2_struct_tag_smp *smp_itm_tag;
    term_str_tag = stivale2_get_tag(stivale2_struct, STIVALE2_STRUCT_TAG_TERMINAL_ID);
    smp_itm_tag = stivale2_get_tag(stivale2_struct, STIVALE2_STRUCT_TAG_SMP_ID);
    ...
    CPUInit();
    printf("CPU Count: %d\n", smp_itm_tag->cpu_count);
    for(int i=0; i<smp_itm_tag->cpu_count; i++)
    {
        CPUInit();
        printf("CPU %d: %d ", i, smp_itm_tag->smp_info[i].target_stack);
        Sleep(1);
